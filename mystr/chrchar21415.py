...
# 새로운 컬럼을 생성하고 계산 (천 단위 구분 기호 제거)
line1_stations['출근시간_승하차_합'] = (
    line1_stations[time_columns[0]].replace(',', '', regex=True).astype(int) +
    line1_stations[time_columns[1]].replace(',', '', regex=True).astype(int)
)
...
# 결과 출력 (천 단위 구분 기호 추가)
line1_stations['출근시간_승하차_합'] = line1_stations['출근시간_승하차_합'].apply(lambda x: "{:,}".format(x))
